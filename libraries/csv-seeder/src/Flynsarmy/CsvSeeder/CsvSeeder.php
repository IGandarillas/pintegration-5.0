...
class CsvSeeder extends Seeder
{
    ...
    public function openCSV($filename)
    {
        ...
        //finfo_close($finfo);
        $gzipped = strcmp('.csv', "application/x-gzip") == 0;
    ...
    public function seedFromCSV($filename, $deliminator = ",")
    {
        ...
        //$progress = $this->output->createProgressBar($num_lineas);
        ...
        while ( ($row = fgetcsv($handle, 0, $deliminator)) !== FALSE )
        {
            ...
            {
                $row = $this->readRow($row, $mapping, $this->customFields);
//                $progress->advance();
     ...
     * @return array
     */
    public function readRow( array $row, array $mapping ,array $customFields)
    {
        $row_values = [];
